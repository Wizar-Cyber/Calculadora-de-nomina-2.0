...
export function NetoCard({ neto, dias, horas, fechaPago, devengado, deducciones, auxilio, civicas }: NetoCardProps) {
  ...
  return (
    <div className="bg-gradient-to-r from-blue-900 to-blue-800 px-3 sm:px-6 md:px-8 py-3 sm:py-5 md:py-6 text-center text-white">
      <p className="text-xs uppercase tracking-[0.2em] sm:tracking-[0.3em] text-white/70">Neto a pagar</p>
      <p className="mt-2 sm:mt-3 text-3xl sm:text-5xl md:text-6xl font-black tabular-nums break-words line-clamp-2">{formatCurrency(neto)}</p>

      <div className="mt-3 sm:mt-5 flex flex-row justify-center gap-3 sm:gap-6 md:gap-8 text-xs sm:text-sm">
        ...
        <div className="text-center">
          <p className="text-white/60 text-xs mb-1">Descuentos</p>
          <p className="text-red-400 font-semibold">{formatCurrency(deducciones)}</p>
        </div>
      </div>

      <div className="mt-2 sm:mt-4 w-full max-w-xs mx-auto px-3 sm:px-0">
